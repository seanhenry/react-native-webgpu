...
Value ContextHostObject::get(Runtime &runtime, const PropNameID &propName) {
    ...
    if (name == "getCurrentTexture") {
        return WGPU_FUNC_FROM_HOST_FUNC(getCurrentTexture, 0, [this]) {
            ...
            if (_texture != nullptr) {
                return Object::createFromHostObject(runtime, std::make_shared<TextureHostObject>(_texture, _configuredContext, "[WEBGPU] SurfaceTexture", true));
            }
            ...
            if (texture.status == WGPUSurfaceGetCurrentTextureStatus_Success) {
                ...
                // [Surface texture note 2] isSurfaceTexture = true so TextHostObject does not release it.
                // We will release it in presentSurface, or keep using it until this is called.
                return Object::createFromHostObject(runtime, std::make_shared<TextureHostObject>(texture.texture, _configuredContext, "[WEBGPU] SurfaceTexture", true));
    ...
    if (name == "presentSurface") {
        return WGPU_FUNC_FROM_HOST_FUNC(present, 0, [this]) {
            ...
            // [Surface texture note 3] assuming we called getCurrentTexture() we have a surface texture.
            // Once the surface is presented, we release it, so we can get the next one.
            if (_texture != nullptr) {
            ...
            jsLog(runtime, "warn", {"Cannot present surface because surface texture doesn't exist. Call context.getCurrentTexture() to create one."});
            return Value::undefined();
        });
    }
