...
class JNIBlobBitmapLoaderMethodIDs {
 public:
  explicit JNIBlobBitmapLoaderMethodIDs(JNIEnv *env) {
    ...
    getSharedMemoryId = env->GetMethodID(loaderClass, "getSharedMemory", "()Landroid/os/SharedMemory;");
...
class JNIBlobBitmapLoader {
 public:
  explicit JNIBlobBitmapLoader(std::shared_ptr<JNIBlobBitmapLoaderMethodIDs> methodIDs, jobject instance)
