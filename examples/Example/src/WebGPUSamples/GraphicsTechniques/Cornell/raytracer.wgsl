...
const NumReflectionRays : f32 = 5.0;
const NumReflectionRaysInt = 5;
...
fn main(@builtin(global_invocation_id) invocation_id : vec3u) {
  if (all(invocation_id.xy < textureDimensions(framebuffer))) {
    ...
    for (var i = 0; i < NumReflectionRaysInt; i++) {
...
fn sample_hit(hit : HitInfo) -> vec3f {
  ...
  return textureSampleLevel(lightmap, smpl, hit.uv, hit.quad, 0.0).rgb +
