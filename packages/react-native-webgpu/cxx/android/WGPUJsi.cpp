...
extern "C" JNIEXPORT jboolean JNICALL Java_com_webgpu_CxxBridge_00024Companion_installJsi(
    JNIEnv *env, jobject obj, jlong jsiRuntimeRef, jobject jsCallInvokerHolder,
    ...
    auto jsCallInvoker{
        alias_ref<CallInvokerHolder::javaobject>{reinterpret_cast<CallInvokerHolder::javaobject>(
            jsCallInvokerHolder)} -> cthis()->getCallInvoker()};

    auto newSurfaces =
        std::make_shared<std::unordered_map<std::string, std::shared_ptr<Surface>>>();
    JSIInstance::instance =
        std::make_unique<JSIInstance>(*runtime, std::make_shared<Thread>(jsCallInvoker));
...
extern "C" JNIEXPORT void JNICALL Java_com_webgpu_CxxBridge_00024Companion_onSurfaceDestroyed(
    JNIEnv *env, jobject obj, jstring inUUID) {
    const char *uuidChars = env->GetStringUTFChars(inUUID, nullptr);
    std::string uuid(uuidChars);
    env->ReleaseStringUTFChars(inUUID, uuidChars);
