...
WGPUTextureViewDescriptor wgpu::makeDefaultWGPUTextureViewDescriptor(Runtime &runtime, Object &desc, WGPUTexture texture) {
    ...
    auto mipLevelCount = WGPU_NUMBER_OPT(desc, mipLevelCount, uint32_t, wgpuTextureGetMipLevelCount(texture) - baseMipLevel);
    ...
    if (arrayLayerCount == 0) {
        if (dimension == WGPUTextureViewDimension_1D || dimension == WGPUTextureViewDimension_2D || dimension == WGPUTextureViewDimension_3D) {
            arrayLayerCount = 1;
