...
import {createBindGroupCluster} from '../../GPGPU/BitonicSort/utils';
import {
...
export const SkinnedMesh = () => {
  ...
  const onCreateSurface: WebGpuViewProps['onCreateSurface'] = async ({
    ...
    */
    ...
    const whaleScene = await fetch(
      `${WEB_GPU_SAMPLES_BASE_URL}/assets/gltf/whale.glb`,
    )
    ...
    function frame() {
      ...
      (
        gltfRenderPassDescriptor.colorAttachments as GPURenderPassColorAttachment[]
      )[0].view = framebuffer.createView();

      (
        skinnedGridRenderPassDescriptor.colorAttachments as GPURenderPassColorAttachment[]
      )[0].view = framebuffer.createView();
