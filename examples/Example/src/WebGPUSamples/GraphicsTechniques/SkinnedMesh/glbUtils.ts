...
export class GLTFBufferView {
  ...
  constructor(buffer: GLTFBuffer, view: BufferView) {
    ...
    this.gpuBuffer = null as unknown as GPUBuffer;
    this.usage = 0;
  }
...
export class GLTFPrimitive {
  ...
  buildRenderPipeline(
    ...
    const vertexBuffers: GPUVertexBufferLayout[] = this.attributes.map(
      (attr, idx) => {
        const vertexFormat: GPUVertexFormat = this.attributeMap[attr]
          .vertexType as GPUVertexFormat;
    ...
    if (this.topology == GLTFRenderMode.TRIANGLE_STRIP) {
      ...
      primitive.stripIndexFormat = this.attributeMap.INDICES
...
export const convertGLBToJSONAndBinary = async (
  ...
  for (const mesh of jsonChunk.meshes!) {
    for (const primitive of mesh.primitives) {
      ...
      for (const attribute of Object.values(primitive.attributes)) {
        ...
        jsonChunk.accessors![attribute].bufferViewUsage! |=
  ...
  for (let i = 0; i < jsonChunk.meshes!.length; i++) {
    ...
    for (let j = 0; j < mesh.primitives.length; ++j) {
      ...
      const primitiveAttributeMap: any = {};
      const attributes = [];
