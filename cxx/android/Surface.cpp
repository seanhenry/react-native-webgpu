...
void Surface::requestAnimationFrame(Function fn) {
    if (_choreographer == nullptr) { return; }
    ...
    if (_callbackData.surface == nullptr) {
        ...
        AChoreographer_postFrameCallback64(_choreographer, wgpuChoreographerFrameCallback,
                                           &_callbackData);
#else
        AChoreographer_postFrameCallback(_choreographer, wgpuChoreographerFrameCallback, &_callbackData);
#endif
...
static void wgpuChoreographerFrameCallback(long frameTimeNanos, void *data) {
#endif
    // Note, in the event that the timer has been invalidated after this callback was scheduled,
    // `animationCallbacks` will be cleared so this callback will not do anything except break
    // the deliberate retain cycle once `callbackData` has been consumed.
