...
export const ABuffer = () => {
  const onCreateSurface: WebGpuViewProps['onCreateSurface'] = async ({
    ...
    const configure = () => {
      ...
      const bytesPerline =
        context.width * averageLayersPerFragment * linkedListElementSize;
      const maxLinesSupported = Math.floor(
        device.limits.maxStorageBufferBindingSize / bytesPerline,
      );
      const numSlices = Math.ceil(context.height / maxLinesSupported);
      const sliceHeight = Math.ceil(context.height / numSlices);
      ...
      const headsBuffer = device.createBuffer({
        size: (1 + context.width * sliceHeight) * Uint32Array.BYTES_PER_ELEMENT,
        usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
        label: 'headsBuffer',
      });
      ...
      (
        opaquePassDescriptor.depthStencilAttachment as GPURenderPassDepthStencilAttachment
      ...
      return function doDraw() {
        // Update the uniform buffer
        {
          ...
          new Uint32Array(buffer, 16 * Float32Array.BYTES_PER_ELEMENT).set([
            averageLayersPerFragment * context.width * sliceHeight,
            context.width,
          ]);
        ...
        (
          opaquePassDescriptor.colorAttachments as GPURenderPassColorAttachment[]
        )[0].view = textureView;
        ...
        for (let slice = 0; slice < numSlices; ++slice) {
          ...
          const scissorWidth = context.width;
          const scissorHeight =
            Math.min((slice + 1) * sliceHeight, context.height) -
            slice * sliceHeight;

          // Draw the translucent objects
          (
            translucentPassDescriptor.colorAttachments as GPURenderPassColorAttachment[]
          )[0].view = textureView;
          ...
          (
            compositePassDescriptor.colorAttachments as GPURenderPassColorAttachment[]
          )[0].view = textureView;
