...
export const SampleInitFactoryWebGPU = async (
  ...
  const init = async ({ context, /* canvas, gui, stats*/ }: SampleInitParams) => {
    const {navigator} = webGPU
...
export abstract class Base2DRendererClass {
  ...
  setUniformArguments<T extends {[key: string]: number}, K extends readonly string[]>(
    device: GPUDevice,
